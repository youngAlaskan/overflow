...
std::vector<Vertex> TerrainGenerator::GenerateVertices(FastNoise::SmartNode<FastNoise::Generator> generator) {	
	...
	generator->GenUniformGrid2D(heightMap, 0, 0, m_ResX + 1, m_ResZ + 1, m_Freq, m_Seed);
	...
	for (GLuint j = 0; j < m_ResZ; j++) {
		for (GLuint i = 0; i < m_ResX; i++) {
			...
			glm::vec4 a = glm::vec4(x, heightMap[idx] * m_HeightMul, z, 0.0f);
			glm::vec4 b = glm::vec4(x + dx, heightMap[idx + 1] * m_HeightMul, z, 0.0f);
			glm::vec4 c = glm::vec4(x, heightMap[idx + w] * m_HeightMul, z + dz, 0.0f);
			glm::vec4 d = glm::vec4(x + dx, heightMap[idx + w + 1] * m_HeightMul, z + dz, 0.0f);
